...
class CausalSelfAttention(torch.nn.Module):
    ...
    def forward(
        ...
        # torch.save(x, "looped.pt")
        q, k, v = self.Wqkv(x).split(self.chunks, dim=2)

        # apply norms
        # print(q.shape)
        # print(self.config.num_key_value_heads * self.config.head_dim,)
...
class SandwichBlock(torch.nn.Module):
    ...
    def forward(
        ...
        # torch.save(self.norm_1(x), "looped.pt")
        # exit()
        attn_out = self.norm_1(self.attn(x, freqs_cis, step_idx, mask, past_key_values))
        # attn_out = self.attn(x, freqs_cis, step_idx, mask, past_key_values)
        # torch.save(attn_out, "looped.pt")
        # exit()
        x = attn_out + x
...
class RavenForCausalLM(RavenPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # torch.save(input_embeds, "looped.pt")
        # exit()
