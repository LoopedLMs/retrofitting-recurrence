...
class RavenForCausalLM(RavenPreTrainedModel, GenerationMixin):
    ...
    def iterate_forward(
        ...
        """Run the full recurrence loop over the core block.

        Initializes the latent state, then repeatedly calls core_block_forward for
        num_steps_no_grad steps (no gradient) followed by num_steps_with_grad steps
        (with gradient), as sampled by randomized_iteration_sampler or provided explicitly.

        Args:
            input_embeds: Fixed prelude output, shape (B, S, n_embd). Passed unchanged to
               every recurrence step as the input injection signal.
            input_states: Optional externally supplied initial latent state, shape (B, S, n_embd).
               When provided (e.g. for continuous/warm-start compute), skips internal
               initialization entirely. When None, the initial state is determined by
               config.initial_state_mode.
            freqs_cis: Precomputed rotary embedding (cos, sin) pair for the current positions.
            block_idx: Running counter of absolute block indices for KV cache keying. Passed
               through to core_block_forward and incremented there.
            mask: Optional FlexAttention BlockMask. None triggers standard causal SDPA.
            past_key_values: KV cache accumulating keys/values across recurrence steps and
               generation steps. None means no caching.
            num_steps: Controls the total number of recurrence iterations. If None, sampled
               stochastically via randomized_iteration_sampler. If a 2-element sequence,
               interpreted as (num_steps_no_grad, num_steps_with_grad). If a scalar,
               all steps are run without gradient.
            init_scale: Scalar multiplier applied to the initial state std when using random
               initialization (config.initial_state_mode == "random").
        """
        if input_states is not None:
            x = xk = input_states.clone()
        elif self.config.initial_state_mode in ("embed-init", "skip-adapter"):
            x = xk = input_embeds.clone()
        else:
    ...
    def core_block_forward(
        ...
        """Run one recurrence step through the core transformer block.

        Args:
            x: The recurrent latent state — evolves across steps. On the first step this is
               the initial state (random noise, zeros, or a copy of input_embeds depending on
               config.initial_state_mode). On subsequent steps it is the output of the
               previous call to this function.
            input_embeds: The fixed prelude output — a per-token representation that does not
               change across recurrence steps. Passed through the adapter at every step so that
               the core block always has access to the original input signal.
            freqs_cis: Precomputed rotary embedding (cos, sin) pair for the current positions.
            mask: Optional FlexAttention BlockMask for causal or padded attention. None triggers
               standard causal SDPA inside each SandwichBlock.
            past_key_values: KV cache (HuginnDynamicCache or HuginnStaticCache). Each
               SandwichBlock writes its keys/values keyed by block_idx, so the cache
               accumulates one entry per block per recurrence step.
            block_idx: Running counter of absolute block indices used as cache keys. Mutated
               in-place (incremented) as each SandwichBlock is visited.
            current_step: Zero-based index of the current recurrence iteration. Controls
               whether input injection is skipped at step 0 under "skip-adapter" mode.
        """
        if current_step == 0 and self.config.initial_state_mode == "skip-adapter":
            x = input_embeds.to(x.device)  # no noise injection, no adapter; step 0 is a pure prelude pass
        else:
