...
def import_times_new_roman(this_font_manager=font_manager, this_plt=plt, font_size=16):
    this_font_manager.fontManager.addfont("paper_plots/Times New Roman.ttf")
...
def get_flops(
    all_counts, train_rec, this_chkpt, max_steps, batch_size=2**20, warmup_duration=0.25, warmup_type="linear"
):  # 1M batch size
    if train_rec == 1:
        ...
        for chkpt in chkpt_dirs:
            ...
            for json_file in json_files_recursive:
                ...
                if dict_name not in model_size_cache.keys():
                    ...
                    if (
                        ("TinyLlama-1.1B-intermediate-step-1431k-3T" in dir)
                        or ("Llama-3.2-1B-untied" in dir)
                        or ("OLMo-2-0425-1B-step1907359" in dir)
                    ):
                    ...
                    if (
                        ...
                        model_size_cache[dict_name] = {
                            1: {"total_not_emb_or_lm_head": total_params, "flops_times_by_6d": total_params}
                        }
                    else:
                        ...
                        all_data.append(
                            {
                                ...
                                "FLOPs": get_flops(
                                    model_size_cache[dict_name],
                                    extra_dict["train_rec"],
                                    this_chkpt,
                                    max_steps=extra_dict.get("max_steps", 25_000),
                                    warmup_duration=extra_dict.get("warmup_duration", 0.25),
                                    warmup_type=extra_dict.get("warmup_type", "linear"),
                                ),
                            }
                        )
    ...
    for task_name in df["task"].unique():
        ...
        for x_axis in x_axes:
            ...
            model_legend_handles = [
                matplotlib.lines.Line2D([0], [0], color=model_colors[model], lw=2, label=model) for model in models
            ]
...
if __name__ == "__main__":
    ...
    runs = [
        (
            "eval_outputs/huginn_llama/16_node_bf16_mixed_mbs_8_bs_16_olmo_2_0425_1b_step1907359_4_6_4_last_6_layer_5e_5_schedule_n_75_cooldown_9_muon_001_nemotron_max_mean_rec_32_sqrt_50k_steps_attempt_2",
            "4,6,4 (Train Recurrence=32)",
            {"train_rec": 32, "max_steps": 50_000, "warmup_duration": 0.75, "warmup_type": "1-sqrt"},
        ),
        (
            "eval_outputs/huginn_llama/16_node_bf16_mixed_mbs_8_bs_16_olmo_2_0425_1b_step1907359_4_6_4_last_6_layer_5e_5_schedule_n_75_cooldown_9_muon_001_nemotron_max_mean_rec_16_sqrt_50k_steps_attempt_2",
            "4,6,4 (Train Recurrence=16)",
            {"train_rec": 16, "max_steps": 50_000, "warmup_duration": 0.75, "warmup_type": "1-sqrt"},
        ),
        (
            "eval_outputs/huginn_llama/16_node_bf16_mixed_mbs_8_bs_16_olmo_2_0425_1b_step1907359_4_6_4_last_6_layer_5e_5_schedule_n_75_cooldown_9_muon_001_nemotron_max_mean_rec_8_sqrt_50k_steps_attempt_2",
            "4,6,4 (Train Recurrence=8)",
            {"train_rec": 8, "max_steps": 50_000, "warmup_duration": 0.75, "warmup_type": "1-sqrt"},
        ),
        (
            "eval_outputs/huginn_llama/16_node_bf16_mixed_mbs_8_bs_16_olmo_2_0425_1b_step1907359_4_6_4_last_6_layer_5e_5_schedule_n_75_cooldown_9_muon_001_nemotron_max_mean_rec_4_sqrt_50k_steps_attempt_2",
            "4,6,4 (Train Recurrence=4)",
            {"train_rec": 4, "max_steps": 50_000, "warmup_duration": 0.75, "warmup_type": "1-sqrt"},
        ),
        (
            "eval_outputs/huginn_llama/16_node_bf16_mixed_mbs_8_bs_16_non_recur_olmo_2_1907359_5e_5_no_compile_cooldown_9_muon_001_nemotron_50k_steps_attempt_2",
            "Olmo-2-step1907359\nStatic Depth",
            {"train_rec": 1},
        ),
